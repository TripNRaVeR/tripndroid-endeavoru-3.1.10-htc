...
show_one(cpu_temp, tdf_cpu_temp);
...
static ssize_t store_cpu_temp(struct kobject *a, struct attribute *b,
				   const char *buf, size_t count)
{
	return 0;
}
define_one_global_rw(cpu_temp);
